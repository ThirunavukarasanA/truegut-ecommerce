...
export default function PincodeChecker() {
     const { pincode: storedPincode, updateLocation, isLocationSet, clearLocation } = useLocation();
     ...
     const handleCheck = async () => {
          ...
          try {
               ...
               if (res.ok && data.serviceable) {
                    ...
                    if (storedPincode && storedPincode !== pincode && cartItems.length > 0) {
                         await clearCart();
     ...
     return (
          <div id="pincode-section" className="my-6 p-4 border border-gray-100 rounded-2xl bg-gray-50/50 space-y-3">
               ...
               <div className="relative flex items-center">
                    ...
                    <button
                         ...
                         className="absolute right-2 px-4 py-1.5 bg-transparent text-primary font-bold text-sm hover:bg-primary/5 rounded-lg transition-colors disabled:text-gray-400 flex items-center justify-center min-w-[60px]"
                    >
                         {loading ? (
                    ...
                    {serviceable === true && !error && (
                         ...
                         >
                              <p className="text-sm font-medium text-gray-800 flex items-center gap-2">
                                   ...
                                   FREE delivery by <span className="text-primary font-bold">{getDeliveryEstimate()}</span>
                              </p>
                              <p className="text-xs text-gray-500 flex items-center gap-1.5">
                                   Looking good! Serviceable at <span className="font-semibold text-gray-700">{pincode}</span>
                              </p>
                         </motion.div>
                    )}
               </AnimatePresence>
               ...
               <div className="flex flex-col gap-3 pt-2">
                    <div className="flex items-center justify-between">
                         ...
                         >
                              ...
                              <span className="underline underline-offset-2">Shipping & Returns</span>
                         </button>

                         <div className="flex items-center gap-3">
                              <div className="flex items-center gap-1.5">
                                   <div className="w-1.5 h-1.5 bg-green-500 rounded-full" />
                                   <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Fast</span>
                              </div>
                              <div className="flex items-center gap-1.5">
                                   <div className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
                                   <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Safe</span>
                              </div>
                         </div>
                    </div>
               </div>
