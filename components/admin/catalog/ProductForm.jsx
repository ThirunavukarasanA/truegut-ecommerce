...
export default function ProductForm({
    ...
    if (editMode && initialData) {
      setFormData({
        ...
        fermentation: {
          ...
          liveCulture:
            initialData.fermentation?.liveCulture !== undefined
              ? initialData.fermentation.liveCulture
              : true,
  ...
  const handleSubmit = async (e) => {
    ...
    // HTML content length check is loose, but let's check basic length
    if (formData.description.length > 10000) {
      toast.error("Description cannot exceed 10000 characters");
      return;
    }

    if (formData.history?.length > 10000) {
      toast.error("History cannot exceed 10000 characters");
      return;
    }
    ...
    if (formData.nutrition?.length > 10000) {
      toast.error("Nutrition cannot exceed 10000 characters");
      return;
    }

    if (
      formData.fermentation.durationDays &&
      Number(formData.fermentation.durationDays) < 0
    ) {
    ...
    if (formData.fermentation.alcoholPercentage !== "") {
      ...
      if (alc < 0 || alc > 100) {
        toast.error("Alcohol percentage must be between 0 and 100");
        return;
      }
    }

    if (formData.shelfLifeDays && Number(formData.shelfLifeDays) < 0) {
      toast.error("Shelf life cannot be negative");
      return;
    }
    ...
    data.append(
      "fermentation.durationDays",
      formData.fermentation.durationDays
    );
    data.append("fermentation.liveCulture", formData.fermentation.liveCulture);
    data.append(
      "fermentation.alcoholPercentage",
      formData.fermentation.alcoholPercentage
    );
  ...
  return (
    <form
      ...
      className="flex flex-col h-full bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden"
    >
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-4 p-1">
                {["draft", "active", "archived"].map((s) => (
                  <label
                    key={s}
                    className="flex items-center gap-2 cursor-pointer bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 hover:bg-gray-100 transition-colors"
                  >
                    <input
                      type="radio"
                      name="status"
              ...
              <label className="flex items-center gap-3 cursor-pointer p-4 rounded-2xl hover:bg-gray-50 transition-colors w-full border border-transparent hover:border-gray-100">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      isSubscriptionAvailable: e.target.checked,
                    })
                  }
                ...
                <div>
                  ...
                  <span className="text-xs text-gray-500">
                    Enable recurring orders for this product
                  </span>
                </div>
              </label>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <AdminSelect
                  ...
                  onChange={(e) =>
                    updateNestedStr("fermentation", "type", e.target.value)
                  }
          ...
          <div className="space-y-6">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        requiresColdShipping: e.target.checked,
                      })
                    }
                    className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300"
                  />
                  <span className="text-sm font-medium text-gray-700">
                    Requires Cold Shipping
                  </span>
                </label>
              </div>
            </div>
            <AdminInput
              ...
              onChange={(e) =>
                updateNestedStr("regulatory", "warnings", e.target.value)
              }
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 gap-4">
            ...
            {previewImages.map((src, index) => (
              <div
                key={`new-${index}`}
                className="aspect-square rounded-xl overflow-hidden relative group border-2 border-indigo-100"
              >
                <img
                  src={src}
                  alt="Preview"
