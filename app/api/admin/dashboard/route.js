...
import { startOfDay, startOfMonth, subMonths, subDays, endOfDay, format } from 'date-fns';
...
export async function GET(req) {
     try {
          ...
          const [
               ...
               Order.aggregate([{ $match: { paymentStatus: 'Paid' } }, { $group: { _id: null, sum: { $sum: '$totalAmount' } } }]),
               Order.aggregate([{ $match: { paymentStatus: 'Paid', createdAt: { $gte: todayStart, $lte: todayEnd } } }, { $group: { _id: null, sum: { $sum: '$totalAmount' } } }]),
               Order.aggregate([{ $match: { paymentStatus: 'Paid', createdAt: { $gte: firstDayThisMonth } } }, { $group: { _id: null, sum: { $sum: '$totalAmount' } } }]),
               Order.aggregate([{ $match: { paymentStatus: 'Paid', createdAt: { $gte: firstDayLastMonth, $lt: firstDayThisMonth } } }, { $group: { _id: null, sum: { $sum: '$totalAmount' } } }]),
               Order.countDocuments(),
               Order.countDocuments({ createdAt: { $gte: firstDayThisMonth } }),
               Order.countDocuments({ createdAt: { $gte: firstDayLastMonth, $lt: firstDayThisMonth } }),
               ...
               TempCart.aggregate([{ $unwind: "$items" }, { $group: { _id: null, total: { $sum: { $multiply: ["$items.price", "$items.quantity"] } } } }]),
               // Expired Stock: Sum of quantity where expiryDate < now
               Batch.aggregate([{ $match: { expiryDate: { $lt: now } } }, { $group: { _id: null, sum: { $sum: '$quantity' } } }]),
          ...
          const orderGrowth = totalOrdersLastMonth === 0 ? (totalOrdersThisMonth > 0 ? 100 : 0) : ((totalOrdersThisMonth - totalOrdersLastMonth) / totalOrdersLastMonth) * 100;
          ...
          const deliveredCount = orderDistribution.find(d => d._id === 'Delivered')?.count || 0;
          const cancelledCount = orderDistribution.find(d => d._id === 'Cancelled')?.count || 0;
          ...
          const lowStockProducts = await Variant.find({ _id: { $in: lowStockVariantIds } })
          ...
          const formattedLowStock = lowStockProducts.map(v => ({
               ...
               stock: variantStockLevels.find(l => l._id.toString() === v._id.toString())?.totalQty || 0,
