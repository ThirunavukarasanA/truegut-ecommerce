  ...
  try {
    ...
    const body = {
      ...
      requiresColdShipping:
        getValue("requiresColdShipping") !== undefined
          ? getValue("requiresColdShipping") === "true"
          : undefined,
      ...
      isSubscriptionAvailable:
        getValue("isSubscriptionAvailable") !== undefined
          ? getValue("isSubscriptionAvailable") === "true"
          : undefined,
    ...
    if (
      fermentation.type !== undefined ||
      fermentation.durationDays !== undefined
    ) {
    ...
    const oldUrls = oldImagesInfo.map((img) =>
    ...
    if (product.images && product.images.length > 0) {
      await Promise.all(
        product.images.map(async (img) => {
          // Handle both string URLs and object structure { url, alt }
